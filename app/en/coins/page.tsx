...
export default function CoinsENPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-10">
      <div className="text-center mb-10">
        <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-3">📊 Top 100 Cryptocurrencies</h1>
        <p className="text-gray-500 text-lg">Live prices · 24h change · Market capitalization</p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <input type="text" placeholder="🔍 Search: Bitcoin, BTC, Ethereum..." value={search}
      ...
      {!loading && !error && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-green-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('gainers')}>
            ...
            <p className="text-xs text-gray-500 mt-1">▲ Gaining today</p>
          </div>
          <div className="bg-red-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('losers')}>
            ...
            <p className="text-xs text-gray-500 mt-1">▼ Losing today</p>
          </div>
        </div>
      )}
      ...
      {loading && <div className="text-center py-20"><div className="inline-block w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mb-4"></div><p className="text-gray-500">Loading data...</p></div>}
      {error && <div className="text-center py-20"><p className="text-4xl mb-3">😕</p><p className="text-gray-500">Failed to load data. Please try again later.</p></div>}
      {!loading && !error && filtered.length === 0 && <div className="text-center py-20 text-gray-400"><p className="text-3xl mb-2">🔍</p><p>Nothing found for &quot;{search}&quot;</p></div>}
      ...
      <p className="text-center text-xs text-gray-400 mt-8">Data: CoinGecko API · Updated hourly · Not financial advice</p>
    </main>
  );
}
