...
export default function CoinsTable({ initialCoins }: CoinsTableProps) {
  const [coins, setCoins] = useState<Coin[]>(initialCoins);
  const [filtered, setFiltered] = useState<Coin[]>(initialCoins);
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState<'all' | 'gainers' | 'losers'>('all');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(initialCoins.length === 0);

  useEffect(() => {
    setCoins(initialCoins);
    setFiltered(initialCoins);
    setError(initialCoins.length === 0);
  }, [initialCoins]);
      ...
      {!error && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-blue-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('all')}>
            <p className="text-2xl font-black text-blue-700">{coins.length}</p>
            <p className="text-sm text-blue-600 font-medium">Монет</p>
          </div>
          <div className="bg-green-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('gainers')}>
            <p className="text-2xl font-black text-green-700">{gainers}</p>
            <p className="text-sm text-green-600 font-medium">Ростуть</p>
          </div>
          <div className="bg-red-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('losers')}>
            <p className="text-2xl font-black text-red-700">{losers}</p>
            <p className="text-sm text-red-600 font-medium">Падають</p>
          </div>
        </div>
      )}

      {/* Table header */}
      <div className="grid grid-cols-12 gap-4 px-4 py-3 text-xs text-gray-500 uppercase tracking-wide border-b border-gray-100 mb-2">
