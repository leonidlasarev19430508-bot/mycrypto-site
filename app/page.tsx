...
const OFFERS=[
  {name:'Binance',id:'binance',description:'Найбільша біржа у світі з низькими комісіями',features:['Низькі комісії','Швидка реєстрація','Надійна платформа'],badge:'Найпопулярніша'},
  {name:'Bybit',id:'bybit',description:'Ідеально для активної торгівлі',features:['Просунуті інструменти','Висока ліквідність','24/7 підтримка'],badge:null},
  {name:'OKX',id:'okx',description:'Сучасна платформа з широкими можливостями',features:['Web3 інтеграція','Стейкінг','Низькі комісії'],badge:null},
];

const ROUTER=process.env.NEXT_PUBLIC_ROUTER_WEBHOOK||'http://localhost:5678/webhook/testgo';
    ...
    <>
      ...
      <main className="p-6 md:p-10 max-w-6xl mx-auto">
        <div className="text-center mt-6 mb-10">
          <h1 className="text-4xl md:text-5xl font-black text-gray-900 leading-tight">Best Crypto Exchanges</h1>
          <p className="text-gray-500 mt-3 text-lg">Compare exchanges and start trading today</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {OFFERS.map(offer=>(
            <div key={offer.id} className={`p-6 border-2 rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow relative ${offer.badge?'border-orange-400':'border-gray-100'}`}>
              ...
              <a href={`${ROUTER}?offer=${offer.id}&source=site&placement=homepage`} className="mt-5 block bg-orange-500 text-white text-center px-4 py-2.5 rounded-xl hover:bg-orange-600 transition font-semibold">Start Trading on {offer.name}</a>
            </div>
          ))}
        </div>
        <CryptoPrices />
