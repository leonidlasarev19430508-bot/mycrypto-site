...
export default function CoinsPLPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-10">
      <div className="text-center mb-10">
        <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-3">📊 Top 100 Kryptowalut</h1>
        <p className="text-gray-500 text-lg">Aktualne ceny · Zmiana 24h · Kapitalizacja rynkowa</p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <input type="text" placeholder="🔍 Szukaj: Bitcoin, BTC, Ethereum..." value={search}
        ...
        <div className="flex gap-2">
          {(['all', 'gainers', 'losers'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? '🌐 Wszystkie' : f === 'gainers' ? '▲ Rosnące' : '▼ Spadające'}
            </button>
          ))}
        </div>
      </div>
      {!loading && !error && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-blue-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('all')}>
            ...
            <p className="text-xs text-gray-500 mt-1">Wszystkich monet</p>
          </div>
          <div className="bg-green-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('gainers')}>
            ...
            <p className="text-xs text-gray-500 mt-1">▲ Rosnące dziś</p>
          </div>
          <div className="bg-red-50 rounded-xl p-4 text-center cursor-pointer" onClick={() => setFilter('losers')}>
            ...
            <p className="text-xs text-gray-500 mt-1">▼ Spadające dziś</p>
          </div>
        </div>
      )}
      <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wide border-b border-gray-100 mb-2">
        ...
        <div className="col-span-4">Moneta</div>
        <div className="col-span-2 text-right">Cena</div>
        <div className="col-span-2 text-right">24h</div>
        <div className="col-span-3 text-right">Kapitalizacja</div>
      </div>
      {loading && <div className="text-center py-20"><div className="inline-block w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mb-4"></div><p className="text-gray-500">Ładowanie danych...</p></div>}
      {error && <div className="text-center py-20"><p className="text-4xl mb-3">😕</p><p className="text-gray-500">Nie udało się załadować danych. Spróbuj później.</p></div>}
      {!loading && !error && filtered.length === 0 && <div className="text-center py-20 text-gray-400"><p className="text-3xl mb-2">🔍</p><p>Nic nie znaleziono dla &quot;{search}&quot;</p></div>}
      ...
      <p className="text-center text-xs text-gray-400 mt-8">Dane: CoinGecko API · Aktualizowane co godzinę · Nie jest to porada finansowa</p>
    </main>
  );
}
